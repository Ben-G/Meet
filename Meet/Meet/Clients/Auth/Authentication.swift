...
struct Authentication {
    ...
    static func saveOAuthAccessPair(pair: OAuthAccessPair) {
        ...
        SSKeychain.setPassword(cachedKey, forService: OAuthKeyConstant,
            account: OAuthAccountConstant)
        SSKeychain.setPassword(cachedSecret, forService: OAuthSecretConstant,
            account: OAuthAccountConstant)
    }

    static func retrieveOAuthAccessPair() -> OAuthAccessPair? {
        let cachedKey = SSKeychain.passwordForService(OAuthKeyConstant,
            account: OAuthAccountConstant)
        let cachedSecret = SSKeychain.passwordForService(OAuthSecretConstant,
            account: OAuthAccountConstant)
    ...
    static func retrieveApplicationAuthPair() -> ApplicationAuthPair {
        let twitterKeysDictionaryURL = NSBundle.mainBundle().URLForResource("TwitterKeys",
            withExtension: "plist")
